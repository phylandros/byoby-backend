...
const getCartItems = [verifyToken, (req, res) => {
...
const addToCart = [verifyToken, (req, res) => {
  const { itemId, quantity, selectedSizeId, selectedColorId } = req.body;
  const userId = req.userId;
...
const deleteCartItem = [verifyToken, (req, res) => {
  const { cartItemId } = req.body;
  console.log(`Attempting to delete cart item with cartItemId: ${cartItemId} for userId: ${req.userId}`);
  ...
  db.query(query, [cartItemId], (err, results) => {
    ...
    if (results.affectedRows === 0) {
      console.log(`No cart item found with id: ${cartItemId}, possibly already deleted.`);
      // Continue to the next item without throwing an error
      return res.status(200).json({ message: `Cart item with id ${cartItemId} not found, possibly already deleted.` });
    }
...
const getTotalPrice = [verifyToken, (req, res) => {
  const userId = req.userId;
  console.log(`Calculating total price for userId: ${userId}`);
...
const updateCartItemQuantity = [verifyToken, (req, res) => {
  const { cartItemId, quantity } = req.body;
  const query = "UPDATE cart_items SET quantity = ? WHERE id = ?";
  db.query(query, [quantity, cartItemId], (err, results) => {
    if (err) {
